...
long long Recursive_NumOneBitsInRange(int a)
{
  // Base Case: a is zero.
  if (a == 0)
    return 0;
  ...
  // 2nd Case: a is even.
  if (a % 2 == 0)
    return __builtin_popcount(a) + Recursive_NumOneBitsInRange(a - 1);
  
  // 3rd Case: a is odd.
  return (((long long)a + 1) / 2) + (2 * Recursive_NumOneBitsInRange(a / 2));
}
...
long long NumOneBitsInRange(int a, int b)
{
  ...
  long long total = (32LL * -(long long)a) - Recursive_NumOneBitsInRange(~a);
  
  // Handle the other number b similarly.
  if (b > 0)
    total += Recursive_NumOneBitsInRange(b);
  else if (b < -1)
  {
    ++b;
    total -= (32LL * -(long long)b) - Recursive_NumOneBitsInRange(~b);
  }
  return total;
}
