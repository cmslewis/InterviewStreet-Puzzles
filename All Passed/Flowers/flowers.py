...
def main():
  # Read in the number of flowers to expect, the number of patrons, and the 
  # prices for all N flowers.
  numFlowers, numPatrons = [int(n) for n in raw_input().split()]
  flowers = [int(n) for n in raw_input().split()]
  
  # Sort the flowers in order of descending cost in preparation for our greedy 
  # approach.
  flowers.sort(reverse=True)
  
  # Because we're using a greedy approach and having the patrons buy flowers in 
  # a cyclic fashion, we only need a single variable to track how many flowers 
  # each patron has purchased (this will be incremented to 0 momentarily). 
  flowersPerPatron = -1
  
  # Now start distributing flowers amongst the patrons in a greedy fashion.
  totalSpent = 0
  for flower_i in range(numFlowers):
    
    # Let the next patron in the lineup purchase this flower.
    patron_i = flower_i % numPatrons
    ...
    # description, taking into account the flower's actual price as well as the 
    # number of flowers this patron has already purchased.
    flowerPrice = (flowersPerPatron + 1) * flowers[flower_i]
