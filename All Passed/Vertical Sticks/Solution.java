...
class Solution
{
  public static void main(String[] args)
  {
    ...
    /* Go ahead and set the specified output precision now, so we can use the 
     * object to format output later.
     */
    ...
    for (int i = 0; i < numCases; ++i)
    {
      ...
      /* Read in the height of each stick into an list. */
      List<Integer> sticks =
        new ArrayList<Integer>(Collections.nCopies(numSticks, 0));
      for (int j = 0; j < numSticks; ++j)
        sticks.set(j, in.nextInt());
      
      /* Print the solution. */
      System.out.println( df.format(ExpectedRayLength(sticks)) );
    }
  }
   ...
   * ---------------------------------------------------------------------------
   * Returns the expected ray length for the provided list of sticks, according 
   * to the problem description. The expected length is computed for each stick 
   ...
   */
  private static double ExpectedRayLength(List<Integer> sticks)
  {
    /* We will return this result as the expected ray length of the given list 
     * of sticks.
     */
    double result = 0;
    ...
    {
      /* Determine the number of sticks with the same height (because the list 
       * has been sorted, they will all be in a contiguous streak).
       */
      int streakLength = 1;
      ...
      {
        int height     = sticks.get(heightRank - 1);
        int nextHeight = sticks.get((heightRank - 1) + 1);
