...
class Solution
{
  ...
  /* Function: getFactors(long x, Set<Long> factors)
   * Usage: Set<Long> factors = new HashSet<Long>();
   *        getFactors(12, factors);
   * ---------------------------------------------------------------------------
   * Updates the provided set to include all factors of the specified number.
   */
  private static void getFactors(long x, Set<Long> factors)
  {
  ...
  public static void main(String[] args)
  {
    ...
    /* Read in the number of unfriendly numbers to expect and the friendly 
     * number. 
     */
    int numUnfriendlies = in.nextInt();
    long friendly = in.nextLong();
    
    /* Read in all of the unfriendly numbers into a list. */
    List<Long> unfriendlies =
      new ArrayList<Long>(Collections.nCopies(numUnfriendlies, (long)0));
    for (int i = 0; i < numUnfriendlies; ++i)
      unfriendlies.set(i, in.nextLong());
    
    /* Before we do anything else, we'll need to find all factors of the 
     * friendly number we've been given. We can do this naively in O(sqrt(N)) 
     * time, where N is the value of the friendly number. This runtime is plenty 
     * sufficient for our purposes.
     */
    Set<Long> friendlyFactors = new HashSet<Long>();
    getFactors(friendly, friendlyFactors);
    
    /* Now we need to find all unfriendly factors that we should disregard 
     * should they appear in our list of friendly factors. The key insight here 
     * is that we don't actually need to factor each unfriendly number in its 
     * entirety; that would simply take too long. Instead, we can optimize our 
     * approach by first finding the greatest common factor (or greatest common 
     * divisor, or GCD) between our friendly number and each unfriendly number, 
     * and then finding the factors of each GCD in much less time. By 
     * definition, the GCD of two numbers is the largest positive integer that 
     * divides the numbers without a remainder, so finding the GCD allows us to 
     * avoid considering any numbers in the interval (GCD, U] for some 
     * unfriendly number U.
     */
    Set<Long> unfriendlyFactors = new HashSet<Long>();
    for (long unfriendly : unfriendlies)
    {
      /* Find the GCD between the friendly number and this unfriendly number. */
      long g = gcd(friendly, unfriendly);
    ...
    /* Finally, we find the difference between the set of friendly factors and 
     * the set of unfriendly factors we accumulated, and print out the size of 
     * this set as the solution.
     */
    friendlyFactors.removeAll(unfriendlyFactors);
    System.out.println(friendlyFactors.size());
  }
}
